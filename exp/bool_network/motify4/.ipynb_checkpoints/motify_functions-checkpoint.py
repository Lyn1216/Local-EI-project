...
def cal_motify(function, file_path):
    ...
    # 找出三元完全子图
    subgraphs, name_subgraphs = function(file_path, graph, 4)
    # 将名称改为序号
...
def make_un_bar_figure(un, figurename):
    motifyname = ["fourone", "bifan", "FBL", "twoFFL", "fiveone", "fivetwo", "fivethree", "sixone", "sixtwo", "sixthree", "sixfour", 
                 "sevenone", "seventwo", "seventhree", "eightone", "eighttwo", "nine", "ten", "eleven", "twelve"]
    # 分组数据
    values_dict = {name: [] for name in motifyname}

    # 收集分组数据
    for key, value in un.items():
        for name in motifyname:
            if name in key:
    ...
    folder_path = 'graph'  
    file_name = f"{str(figurename)}_un Mean and Standard Deviation.png"  
...
def make_un_en_bar_figure(un_en, figurename):
    ...
    # ax.legend(loc='upper right')
    folder_path = 'graph'  
    file_name = f"{str(figurename)}_un_en Mean and Standard Deviation.png"  
    full_path = os.path.join(folder_path, file_name)
    plt.savefig(full_path, dpi=600)
    # 显示图形
    #plt.show()
...
def make_syn_bar_figure(syn, figurename):
    ...
    # ax.legend(loc='upper right')
    folder_path = 'graph'  
    file_name = f"{str(figurename)}_syn Mean and Standard Deviation.png"  
    full_path = os.path.join(folder_path, file_name)
    plt.savefig(full_path, dpi=600)
    # 显示图形
    #plt.show()
...
def make_combined_bar_figure(un, un_en, syn, figurename):
    ...
    # 绘制un和un_en数据集的柱状图
    bars_un = ax2.bar(x, un_means, width, yerr=un_stds, error_kw={'capsize': 3, 'elinewidth': 1, 'ecolor': 'grey'}, color='#74B38F', label='Un Values')
    bars_un_en = ax2.bar(x + width, un_en_means, width, yerr=un_en_stds, error_kw={'capsize': 3, 'elinewidth': 1, 'ecolor': 'grey'}, color='#9B76B2', label='Un_en Values')
    ...
    ax2.set_ylabel('Un and Un_en Values')
