...
def text_bn_graph(textfile = 'example.txt', condidate_sys=None, fill_onenode=False, noise=0, save_onenote = False):
    ...
    #pos = nx.circular_layout(G)   #圆周布局
    #nodes_in_each_layer = [5, 3, 2]   #同心圆，3层，每层个数分别为5，3，2
    #pos = nx.shell_layout(G, nodes_in_each_layer)
    #pos = nx.kamada_kawai_layout(G)   #力导向布局
    #pos = nx.spectral_layout(G)   #相邻节点近
    ...
    if condidate_sys is not None:
        ...
        print(tpm)
        un = unique(tpm, len(condidate_sys), len(neigbors)-len(condidate_sys))[0]
        print("un:  " + str(unique(tpm, len(condidate_sys), len(neigbors)-len(condidate_sys))[0]))
        syn = synergy(tpm, len(condidate_sys), len(neigbors)-len(condidate_sys))
        print("syn:  " + str(synergy(tpm, len(condidate_sys), len(neigbors)-len(condidate_sys))))
        vividness = condi_ei(tpm, len(condidate_sys), len(neigbors)-len(condidate_sys))
        print("vividness:  " + str(condi_ei(tpm, len(condidate_sys), len(neigbors)-len(condidate_sys))))
        print(120 * '-')
    
    return tpm, un, syn, vividness, onenote_tpm_result, onenote_un_result, onenote_syn_result, onenote_vividness_result
